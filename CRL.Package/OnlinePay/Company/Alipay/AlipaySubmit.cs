...
namespace CRL.Package.OnlinePay.Company.Alipay
{
    public class AlipaySubmit
    {
        ...
        public static string BuildRequest(SortedDictionary<string, string> sParaTemp, string strMethod, string strButtonValue)
        {
            ...
            sbHtml.Append("<form id='alipaysubmit' target='_blank' name='alipaysubmit' action='" + GATEWAY_NEW + "_input_charset=" + _input_charset + "' method='" + strMethod.ToLower().Trim() + "'>");
        ...
        public static string BuildRequest(SortedDictionary<string, string> sParaTemp)
        {
            ...
            //CoreHelper.EventLog.Log("支付宝退款参数为:" + strRequestData);
            //把数组转换成流中所需字节数组类型
